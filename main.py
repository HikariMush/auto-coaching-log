...
# GitHub Actions上では.envがないので環境変数から直接読む
# load_dotenv() # ローカルテスト用
...
def mix_audio_files(file_paths):
    ...
    logging.info(f"Mixing {len(file_paths)} files...")
...
def analyze_audio(file_path):
    ...
    prompt = """
    以下の音声はコーチングセッションの録音です（複数人の声が混ざっています）。
    以下の情報を抽出し、JSON形式のみを出力してください。
    
    {
      "student_name": "生徒の名前（呼びかけから推測。Control Centerと一致させる）",
      "date": "YYYY-MM-DD",
    ...
    else: raise ValueError("Failed to parse JSON")

def main():
    # 1. Inboxにあるファイルを取得 (フォルダ以外)
    ...
    # 全てダウンロードして合成 (簡易ロジック: Inboxにあるものは1つのセッションとみなす)
    # ※GitHub Actionsは15分ごとなので、通話終了後の実行を想定
    local_paths = []
    file_ids = []
    ...
    try:
        ...
        if cc_res:
            ...
            if target_id_prop:
                ...
                move_files_to_processed(file_ids, processed_folder_id)
